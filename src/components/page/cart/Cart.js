...
import { CartContainer, ProductItem, TitleDiv, TotalPrice, ItemImage, CartBrandName, OrderTime, OrderStatus, CartDiv} from './styles'
...
export default function Cart() {
  ...
  useEffect(() => {
    ...
    const getCartArrInfo = async () => {
      const cart = await getCart(user.email).then(res => res.data.reverse())
      setValues({
        ...values,
        cartArr: cart
      });
      ...
      <CartContainer>
        ...
        {values.cartArr.map(cart => 
          ...
          (cart.isCaseClosed === true &&
            moment().isAfter(moment(cart.date, 'YYYY-MM-DDTHH:mm:ss.SSSZ').add(2, 'hours')) 
            ?
            <div key={cart._id}/>
            :
            <div key={cart._id}>
